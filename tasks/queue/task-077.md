...
- 交付结果：
  新增 `pnpm ai:context-retro` 与 `context_packet` tracking；`build-context` 默认不再拼全文，`--expanded` 才附 excerpt；Studio 服务端改为只读派生 JSON，不直接依赖脚本执行层；task 分支回收补齐 `backfill/report/cleanup` 命令、release 触发与任务页/首页只读治理提示；GitHub remote 接入前可通过 readiness 摘要看到还差哪一步。
- 复盘：
