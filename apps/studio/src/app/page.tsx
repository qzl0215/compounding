...
export default async function HomePage() {
  ...
  const outline = [
    ...
    { id: "current-focus", label: "当前主线" },
    { id: "task-board", label: "当前待办" },
    { id: "memory-and-decisions", label: "记忆与决策" },
    { id: "project-index", label: "项目索引" },
    { id: "agent-roles", label: "代理职责" },
    { id: "git-health", label: "改动前检查" },
  ];

  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_260px]">
      <div className="space-y-6">
        <section id="project-intro" className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          <Card>
            ...
            <p className="mt-4 max-w-3xl text-lg text-white/68">
              当前仓库已经收口成 AI-Native Repo。规则走 <code>AGENTS.md</code>，长文走 <code>docs/*</code>，项目记忆走
              <code>memory/*</code>，上下文入口走 <code>code_index/*</code>，每次可合并改动都绑定到 <code>tasks/*</code>。
            </p>
          ...
          <Card>
            ...
            <h3 className="mt-2 text-2xl font-semibold">主源压缩镜像</h3>
            <SectionContent content={overview.projectIntro} empty="项目介绍区块暂未生成。" />
          </Card>
        </section>

        <section id="current-focus" className="grid gap-6 xl:grid-cols-[0.62fr_0.38fr]">
          <Card>
        ...
        <section id="task-board" className="grid gap-6 xl:grid-cols-[0.56fr_0.44fr]">
          <Card>
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-accent">当前待办</p>
                <h3 className="mt-2 text-2xl font-semibold">任务清单摘要</h3>
              </div>
              <Link
                href="/tasks"
              ...
              {overview.tasks.length > 0 ? (
                overview.tasks.map((task) => (
                  ...
                  >
                    ...
                    <p className="mt-3 text-xs text-white/52">{task.updateTrace}</p>
                  </Link>
                ))
              ) : (
          ...
          <section id="memory-and-decisions">
            <Card>
