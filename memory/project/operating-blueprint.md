...
继续把结构收口落到真正会制造熵增的边界上：`t-058` 与 `t-059` 已经把 CLI 外壳和 release/task 状态链收正，`t-061` 已把 `portal` 读模型聚合层收成薄 barrel，当前主线切到 `t-062`，继续收薄 `portal` 首页 shell，把 `home-dashboard.tsx` 拆成薄入口与更小的面板模块；目标仍是减少对象歧义和重复外壳，而不是增加新框架。
