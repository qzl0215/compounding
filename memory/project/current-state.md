...
- 当前运营重点仍是 `t-044`；同时插入一条直接执行 task `t-047`，把任务页收成单一子任务表格，避免执行页继续长成离散工作台。
...
- 推进 `t-044` 的 task 创建前置逻辑与 Search Before Building 规则落地
- 验证 `t-047` 是否已把任务页收成单一子任务表格，而不是继续按区块重复解释
