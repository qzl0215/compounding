...
该模板用于 `integration_report_template` 场景下的标准输入输出，默认服从项目汇报契约。
...
- File: `docs/60_TEMPLATES/integration_report_template.md`
