...
该模板用于 `report_default_template` 场景下的标准输入输出，默认服从项目汇报契约。
...
- File: `docs/60_TEMPLATES/report_default_template.md`
