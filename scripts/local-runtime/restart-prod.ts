...
async function main() {
  ...
  if (status.status === "stopped" || status.status === "stale_pid") {
    console.log(JSON.stringify({ ok: true, message: "本地生产当前未运行，按约定不自动拉起。", status }, null, 2));
    return;
  }
...
main().catch((error) => {
  console.log(JSON.stringify({ ok: false, message: error instanceof Error ? error.message : "本地生产重启失败。" }, null, 2));
  process.exit(1);
});
