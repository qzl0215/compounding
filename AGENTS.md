...
- 只允许一层 plan；`memory/project/goals.md` 是唯一 Goal 主源，`memory/project/current-state.md` 只保留运营快照。
...
- 需求不清、范围不清或发布标准不清时，先回到 `memory/project/goals.md` 收口，不得直接创建执行 task。
...
- 再读 `memory/project/goals.md`、`memory/project/current-state.md`。
