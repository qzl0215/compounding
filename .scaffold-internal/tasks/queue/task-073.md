...
- 交付结果：
  共享 judgement contract 已复用到 project-state 与 feature-context；新增 `shared/project-judgement-live.ts` 聚合 live task/release/runtime facts；judgement contract validator 现在对同一份 live judgement 做比对；operator runbook / tool entry 补上三模式与默认 AI feature 入口；最小 golden matrix 已覆盖 cold_start / normalize / ai_upgrade。
- 复盘：
