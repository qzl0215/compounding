---
title: 收口 release 单一状态机与 registry 投影
task_id: task-099
type: structural
status: done
...
- 父计划：`memory/project/goals.md`
- 任务摘要：收口 release 单一状态机与 registry 投影
...
- 状态：已完成
- 体验验收结果：`pnpm validate:static` 通过，139 tests 通过
- 交付结果：`kernel/release-state-machine.yaml` + `shared/release-state-machine.ts` 成为唯一 release 状态真相；`shared/release-registry.ts` 只做投影/修复；`scripts/release/*`、Studio、harness 与 project judgement 统一读 `state_id` / `state_label`；已回写 `Current`。
- 复盘：单一状态机已落地，registry 投影已收口，写链和读链已统一。
...
- `apps/studio/src/modules/releases/__tests__/state-machine.test.ts`
